...
createApp({
    data() {
        return {
            ...
            deletePollTimers: {},
            deleteRemoveTimers: {},
            token: 'local_admin|admin',
            currentUser: { username: 'local_admin', role: 'admin' }
        };
    },
    computed: {
        isAuthenticated() {
    ...
    methods: {
        ...
        handleLogout() {
            ...
            this.token = 'local_admin|admin';
            this.currentUser = { username: 'local_admin', role: 'admin' };
            this.messages = [];
            this.sessions = [];
            this.documents = [];
            this.skills = [];
        ...
        async handleSend() {
            ...
            try {
                        ...
                        if (eventStr.startsWith('data: ')) {
                            ...
                            try {
                                ...
                                if (data.type === 'content') {
                                    ...
                                    botMessage.isThinking = false;
                                    botMessage.text = data.content || '(重复请求已忽�?';
                                }
                            } catch (e) {
                ...
                if (!receivedDone) {
                    if (!botMessage.text) {
                        botMessage.text = '(回答已中断，可重新生�?';
                    } else if (!botMessage.text.includes('回答已中断，可重新生�?)) {
                        botMessage.text += '\n\n_(回答已中断，可重新生�?_';
                    }
                }
        ...
        async deleteMcpServer(server) {
            if (!confirm(`确定要删�?MCP 配置 "${server.name}" 吗？`)) {
                return;
            }
            try {
        ...
        createUploadSteps() {
            return [
                ...
                { key: 'cleanup', label: '清理旧版�?, percent: 0, status: 'pending', message: '' },
                { key: 'parse', label: '解析与分�?, percent: 0, status: 'pending', message: '' },
                { key: 'parent_store', label: '父级分块入库', percent: 0, status: 'pending', message: '' },
                { key: 'vector_store', label: '向量化入�?, percent: 0, status: 'pending', message: '' },
            ];
        },
        ...
        uploadFileWithProgress(file) {
            return new Promise((resolve, reject) => {
                ...
                xhr.upload.onprogress = (event) => {
                    ...
                    this.updateUploadStep('upload', percent, 'running', `已上�?${percent}%`);
                };

                xhr.onload = () => {
                    if (xhr.status === 401) {
                        this.handleLogout();
                        reject(new Error('登录已过期，请重新登�?));
                        return;
                    }
                ...
                xhr.onabort = () => reject(new Error('上传已取�?));
                xhr.send(formData);
            });
        },
        ...
        syncUploadJob(job) {
            ...
            // 入库成功后自动收起步骤明细，保留摘要供用户再次展开查看�?            if (job.status === 'completed') {
        ...
        syncDeleteJob(filename, job) {
            ...
            // 后端返回统一的步骤结构，前端只负责同步到当前文档行内卡片�?            this.setDeleteJob(filename, {
                jobId: job.job_id,
                status: job.status,
                message: job.message || '',
        ...
        scheduleDeletedDocumentRemoval(filename) {
            ...
            // 删除完成后先保留 3 秒摘要，再从当前列表移除并刷新后端状态�?            const timer = setTimeout(async () => {
        ...
        async deleteDocument(filename) {
            ...
            if (!confirm(`确定要删除文�?"${filename}" 吗？这将同时删除Milvus中的所有相关向量。`)) {
                return;
            }
